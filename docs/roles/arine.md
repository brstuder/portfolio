# Improving Docs Workflows with AI at Arine
...
The sheer bulk of the work required removal of any unique or advanced style elements, then converting the content into a basic, readable format. I finally applied CSS across the entire knowledge base, reducing the manual work that had been going into formatting PDFs. Overall, the project accomplished a number of feats the KB team had hoped for:

* The KB team could now own the publishing process itself (previously a full stack responsibility), allowing the team to draft and publish documentation in a CI/CD pipeline instead of only during major software releases.
* I reduced total turnaround time (time from drafting content, SME review, and publishing) by over half by completely removing the manual, tedious processes.
